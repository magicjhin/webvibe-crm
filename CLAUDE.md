...
| Styling | Tailwind CSS v4 (CSS-based config через `@theme inline`) |
| UI kit | shadcn/ui v4 (radix base; копируем в `components/ui/`). `form` отсутствует в v4 — используем кастомный `components/forms/Field.tsx` |
| Icons | lucide-react |
| DB | Postgres @ Neon (eu-central-1) |
| ORM | Prisma 7. **Runtime** — `@prisma/adapter-neon` (WebSocket, Neon scale-to-zero на Vercel; ADR-019 Revision 2026-05-31). **Seed** — `@prisma/adapter-pg` (локальный DDL). Generated client в `lib/generated/prisma/`, импорт `@/lib/generated/prisma/client`. URLs в `prisma.config.ts`, не в schema. |
| Auth | Auth.js v5 (NextAuth) Credentials + JWT, split config (ADR-020): `lib/auth.config.ts` Edge-safe для middleware + `lib/auth.ts` Node-only для всего остального |
| Password | `bcryptjs` v3 (типы встроены, `@types/bcryptjs` deprecated и НЕ ставим) |
| Forms | React Hook Form + Zod (`zodResolver`) |
| Tables | TanStack Table |
| Dates | date-fns + date-fns-tz (`Europe/Vilnius`) |
| PDF | @react-pdf/renderer (server-side, Node runtime) — Iter 3+ |
| Signature | react-signature-canvas — Iter 4 |
| Storage | Vercel Blob — Iter 4 |
| Animations | Framer Motion (точечно) |
| Money | `Prisma.Decimal` + `Intl.NumberFormat('lt-LT', { currency: 'EUR' })` |
| PWA | ручной `public/manifest.webmanifest` + production-only SW (Iter 6) |
| Package manager | **pnpm 11** через corepack (`pnpm@11.4.0` в `packageManager`). native postinstall packages whitelisted в `pnpm-workspace.yaml` |
