...
9. **Даты** — `DateTime` (UTC) в БД. При парсинге пользовательского ввода (например, `dueAt` из формы как `"2026-06-15"`) использовать `date-fns-tz` с фиксированной TZ `Europe/Vilnius` (см. `ARCHITECTURE.md` § Dates) — без этого вечерние/ночные ввода могут уехать на день. Конвертация в локальное отображение — также через `date-fns-tz` на стороне рендера.
