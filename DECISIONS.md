...
- **Status:** Accepted (2026-05-28, during Iter 1); **Revised 2026-05-31** — runtime driver сменён на `@prisma/adapter-neon` (см. Revision ниже).
...
- **Alternatives:**
  - `@prisma/adapter-neon` — изначально отклонено, **пересмотрено 2026-05-31** (см. Revision).
...
- **Revision (2026-05-31): runtime driver `@prisma/adapter-pg` → `@prisma/adapter-neon`**
  - **Симптом:** на Vercel (Production) логин падал с «Неверный email или пароль». Реальная причина из Runtime Logs — `CallbackRouteError` → `PrismaClientKnownRequestError: Can't reach database server` (P1001) внутри `prisma.user.findMany()`. С локальной машины та же строка подключения и та же база работают; падает только из serverless-функции.
  - **Корень:** связка `@prisma/adapter-pg` (сырой TCP) + Neon scale-to-zero (Free, sleep через 5 мин) + Vercel serverless. TCP-коннект в момент пробуждения compute рвётся → P1001. Исходное предположение ADR «Node runtime → adapter-neon без выгоды» оказалось неверным для serverless cold start. IP Allow исключён (Free, public internet on).
  - **Решение:** **runtime** (`lib/db.ts`) переведён на `new PrismaNeon({ connectionString: DATABASE_URL })` (`@prisma/adapter-neon` поверх `@neondatabase/serverless`, WebSocket). В Node нет глобального `WebSocket` → `neonConfig.webSocketConstructor = ws`. Драйвер корректно переживает пробуждение спящей базы.
  - **seed остаётся на `@prisma/adapter-pg`** (`prisma/seed.ts`): локальный DDL/seed через `DIRECT_URL`, serverless cold start там не при чём; `pg` + `@prisma/adapter-pg` оставлены в зависимостях ради него.
  - **migrate** не затронут — CLI коннектится сам через `prisma.config.ts` `datasource.url = DIRECT_URL`.
  - **Регион:** добавлен `vercel.json` `regions: ["fra1"]` — функции в одном регионе с Neon `eu-central-1`, минимум сетевой задержки.
  - **Verified:** локальный прогон логики `authorize` через `PrismaNeon` против прод-базы — соединение есть, пароль матчится; `typecheck` + `build` зелёные.
...
## ADR-028: Редактирование и удаление документов разрешено в статусе `sent`, не только `draft`

- **Status:** Accepted (2026-06-11)
- **Context:** Изначально (ADR-026/027 эпоха) счёт и договор можно было редактировать и удалять только в статусе `draft`. На практике владелец часто создаёт счёт сразу со статусом «Отправлен» (`sent`), а потом находит опечатку в разделе **Paslaugos** (строки услуг — за что берутся деньги). Так как кнопки «Редактировать»/«Удалить» показывались только для `draft`, документ оказывался «заморожен» сразу после создания, хотя он ещё никем не подписан и не оплачен. Для single-user CRM, где владелец сам и автор, и единственный оператор, это слишком жёстко.
- **Decision:** Окно правок расширено с `draft` до **`draft` + `sent`** для **счетов** и **договоров**:
  - **Счёт:** editable/deletable в `draft`, `sent`. Заморожен в `paid` (деньги получены) и `cancelled` (terminal).
  - **Договор:** editable/deletable в `draft`, `sent`. Заморожен в `signed` (подпись клиента вшита в PDF — неприкосновенна) и `cancelled`.
  - Правило одной фразой: *«править можно, пока документ не финализирован — не подписан, не оплачен, не отменён»*.
  - Реализация: `updateInvoice`/`updateContract` row-lock через `updateMany WHERE status IN (draft, sent)`; lock-write переведён со `status` на `pdfUrl: null` (иначе sent ошибочно сбрасывался в draft). UI-гейты (`canEdit`, dropdown-удаление, edit-страницы redirect) синхронно расширены.
  - Статус при content-правке **не меняется**: sent остаётся sent. Активный sign-токен договора сохраняется — `/sign/[token]` рендерит договор из БД, клиент видит актуальную версию.
  - КП (`Proposal`) намеренно **не затронуты** — остаются draft-only до отдельного запроса.
- **Consequences:**
  - Владелец может исправить опечатку в Paslaugos/пунктах уже отправленного документа без пересоздания и без потери номера.
  - Корректирует более ранний принцип «documents freeze after creation» из CLAUDE.md: заморозка теперь привязана к финализации (подпись/оплата), а не к факту отправки.
  - Удаление `sent`-документа оставляет «дыру» в нумерации — это ок: счётчики в `Settings` только инкрементятся, `MAX(number)+1` не используется (см. правила нумерации).
- **Alternatives:**
  - Дать менять и номер документа — отклонено: владелец явно сказал, что номер трогать не нужно, только содержимое.
  - Разрешить правки в `signed`/`paid` — отклонено: ломает аудит и неприкосновенность подписи/факта оплаты.
  - «Расфризить» через возврат sent → draft — отклонено: усложняет (договору понадобился бы новый sign-токен), статус при правке не трогаем.
