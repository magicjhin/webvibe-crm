...
/** Жирные отрезки run'а оборачиваем в clauseStrong, остальное — простой текст. */
function ClauseRuns({ runs }: { runs: ClauseRun[] }) {
  return (
    <>
      {runs.map((r, i) =>
        r.bold ? (
          <Text key={i} style={pdfStyles.clauseStrong}>
            {r.text}
          </Text>
        ) : (
          r.text
...
/** Рендерит общий список ClauseSection (см. lib/documents/contractClauses). */
function ClauseSections({ sections }: { sections: ClauseSection[] }) {
  return (
    <>
      {sections.map((sec) => {
        const Wrapper = sec.breakable ? SectionWrap : Section;
        return (
          <Wrapper heading={sec.heading} key={sec.heading}>
            {sec.blocks.map((blk, i) =>
              blk.type === "bullet" ? (
                <Bullet key={i}>
                  <ClauseRuns runs={blk.runs} />
                </Bullet>
              ) : (
                <Para key={i}>
                  <ClauseRuns runs={blk.runs} />
  ...
  return (
    <Document title={`Paslaugu teikimo sutartis ${data.number}`}>
      <Page size="A4" style={pdfStyles.page} wrap>
        ...
        {/* §4–§12 фиксированный boilerplate (общий источник с Word) */}
        <ClauseSections
          sections={projectBoilerplateClauses(data.warranty || DEFAULT_WARRANTY)}
        />
  ...
  return (
    <Document title={`Technines prieziuros sutartis ${data.number}`}>
      <Page size="A4" style={pdfStyles.page} wrap>
        ...
        {/* §2–§5 — общий источник с Word (lib/documents/contractClauses). */}
        <ClauseSections
          sections={maintenanceClauses({
